...
hbb_name = 'udalenka' + ('.exe' if windows else '')
...
def generate_control_file(version):
    ...
    content = """Package: udalenka
Section: net
Priority: optional
Version: %s
Architecture: %s
Maintainer: udalenka <info@rustdesk.com>
Homepage: https://github.com/x6ta/udalenka
...
def build_flutter_deb(version, features):
    ...
    system2('mkdir -p tmpdeb/usr/share/udalenka')
    system2('mkdir -p tmpdeb/etc/udalenka/')
    system2('mkdir -p tmpdeb/etc/pam.d/')
    system2('mkdir -p tmpdeb/usr/share/udalenka/files/systemd/')
    ...
    system2(
        'cp ../res/udalenka.service tmpdeb/usr/share/udalenka/files/systemd/')
    system2(
        'cp ../res/128x128@2x.png tmpdeb/usr/share/icons/hicolor/256x256/apps/udalenka.png')
    system2(
        'cp ../res/scalable.svg tmpdeb/usr/share/icons/hicolor/scalable/apps/udalenka.svg')
    system2(
        'cp ../res/udalenka.desktop tmpdeb/usr/share/applications/udalenka.desktop')
    system2(
        'cp ../res/udalenka-link.desktop tmpdeb/usr/share/applications/udalenka-link.desktop')
    system2(
        'cp ../res/startwm.sh tmpdeb/etc/udalenka/')
    system2(
        'cp ../res/xorg.conf tmpdeb/etc/udalenka/')
    system2(
        'cp ../res/pam.d/udalenka.debian tmpdeb/etc/pam.d/udalenka')
    system2(
        "echo \"#!/bin/sh\" >> tmpdeb/usr/share/udalenka/files/polkit && chmod a+x tmpdeb/usr/share/udalenka/files/polkit")

    system2('mkdir -p tmpdeb/DEBIAN')
    generate_control_file(version)
    system2('cp -a ../res/DEBIAN/* tmpdeb/DEBIAN/')
    md5_file_folder("tmpdeb/")
    system2('dpkg-deb -b tmpdeb udalenka.deb;')

    system2('/bin/rm -rf tmpdeb/')
    system2('/bin/rm -rf ../res/DEBIAN/control')
    os.rename('udalenka.deb', '../rustdesk-%s.deb' % version)
    os.chdir("..")
...
def build_flutter_dmg(version, features):
    ...
    system2('cp -rf ../target/release/service ./build/macos/Build/Products/Release/Удалёнка.app/Contents/MacOS/')
    '''
    system2(
        "create-dmg --volname \"Удалёнка Installer\" --window-pos 200 120 --window-size 800 400 --icon-size 100 --app-drop-link 600 185 --icon Удалёнка.app 200 190 --hide-extension Удалёнка.app rustdesk.dmg ./build/macos/Build/Products/Release/Удалёнка.app")
...
def main():
    ...
    if windows:
        ...
        system2('mv target/release/rustdesk.exe target/release/Удалёнка.exe')
        pa = os.environ.get('P')
        if pa:
            ...
            print('Not signed')
        system2(
            f'cp -rf target/release/Удалёнка.exe {res_dir}')
        os.chdir('libs/portable')
        system2('pip3 install -r requirements.txt')
        system2(
        ...
        if flutter:
            if osx:
                ...
                # system2(
                #     'mv target/release/bundle/deb/rustdesk*.deb ./flutter/udalenka.deb')
                build_flutter_deb(version, features)
        else:
            system2('cargo bundle --release --features ' + features)
            if osx:
                system2(
                    'strip target/release/bundle/osx/Удалёнка.app/Contents/MacOS/rustdesk')
                system2(
                    'cp libsciter.dylib target/release/bundle/osx/Удалёнка.app/Contents/MacOS/')
    ...
    #rcodesign sign --p12-file ~/.p12/rustdesk-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/Удалёнка.app/Contents/MacOS/rustdesk
    #rcodesign sign --p12-file ~/.p12/rustdesk-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/Удалёнка.app/Contents/MacOS/libsciter.dylib
    #rcodesign sign --p12-file ~/.p12/rustdesk-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/Удалёнка.app
    # goto "Keychain Access" -> "My Certificates" for below id which starts with "Developer ID Application:"
    codesign -s "Developer ID Application: {0}" --force --options runtime  ./target/release/bundle/osx/Удалёнка.app/Contents/MacOS/*
    codesign -s "Developer ID Application: {0}" --force --options runtime  ./target/release/bundle/osx/Удалёнка.app
    '''.format(pa))
                system2(
                    'create-dmg "Удалёнка %s.dmg" "target/release/bundle/osx/Удалёнка.app"' % version)
                os.rename('Удалёнка %s.dmg' %
    ...
    # verify:  spctl -a -t exec -v /Applications/Удалёнка.app
    '''.format(pa, version))
                else:
                    print('Not signed')
            else:
                # build deb package
                system2(
                    'mv target/release/bundle/deb/rustdesk*.deb ./udalenka.deb')
                system2('dpkg-deb -R udalenka.deb tmpdeb')
                system2('mkdir -p tmpdeb/usr/share/udalenka/files/systemd/')
                system2('mkdir -p tmpdeb/usr/share/icons/hicolor/256x256/apps/')
                system2('mkdir -p tmpdeb/usr/share/icons/hicolor/scalable/apps/')
                system2(
                ...
                system2(
                    'cp res/udalenka-link.desktop tmpdeb/usr/share/applications/udalenka-link.desktop')
                os.system('mkdir -p tmpdeb/etc/udalenka/')
                os.system('cp -a res/startwm.sh tmpdeb/etc/udalenka/')
                os.system('mkdir -p tmpdeb/etc/X11/rustdesk/')
                os.system('cp res/xorg.conf tmpdeb/etc/X11/rustdesk/')
                os.system('cp -a DEBIAN/* tmpdeb/DEBIAN/')
                os.system('mkdir -p tmpdeb/etc/pam.d/')
                os.system('cp pam.d/udalenka.debian tmpdeb/etc/pam.d/udalenka')
                system2('strip tmpdeb/usr/bin/rustdesk')
                system2('mkdir -p tmpdeb/usr/share/udalenka')
                system2('mv tmpdeb/usr/bin/rustdesk tmpdeb/usr/share/udalenka/')
                system2('cp libsciter-gtk.so tmpdeb/usr/share/udalenka/')
                md5_file_folder("tmpdeb/")
                system2('dpkg-deb -b tmpdeb udalenka.deb; /bin/rm -rf tmpdeb/')
                os.rename('udalenka.deb', 'rustdesk-%s.deb' % version)
